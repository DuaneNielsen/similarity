...
def test_inner_prod():
    ...
    sip = m.similarity_inner_product(f, x, x)
    print(sip)
...
def test_normed_inner_prod():
    ...
    snip = m.similarity_normed_inner_product(f, x, x)
...
def test_normed_inner_prod_dist():
    ...
    y = torch.randn(40, 16)
    f = nn.Sequential(nn.Linear(16, 8), nn.Sigmoid(), nn.Linear(8, 4))
    snip = m.similarity_normed_inner_product(f, x, y)
    print(snip)
...
def test_trace_mean():
    x = torch.randn(40, 16)
    f = nn.Sequential(nn.Linear(16, 8), nn.Sigmoid(), nn.Linear(8, 4))
    dist = m.similarity_trace_mean(f, x, x)
    print(dist)
